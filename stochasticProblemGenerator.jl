...
function add_unsupplied_counter_constraint(stoch_prob, n_unsupplied, network, proba)
    @constraint(stoch_prob.model, unsupplied_cnt, sum(proba .* [sum(stoch_prob.unsupplied[:,n] for n = 1:network.N)]))
end